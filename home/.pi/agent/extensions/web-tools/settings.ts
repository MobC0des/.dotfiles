...
/** Read EXA_API_KEY from the environment, if set. */
export function getExaApiKey(): string | undefined {
	return process.env["EXA_API_KEY"] || undefined;
}

/**
 * Return the Exa MCP endpoint and auth header.
 * Uses Exa's own hosted MCP server when EXA_API_KEY is set,
 * otherwise falls back to dmmulroy's proxy.
 */
export function getExaEndpointConfig(): { endpoint: PublicHttpUrl; authHeader?: string } {
	const apiKey = getExaApiKey();
	if (apiKey) {
		return {
			endpoint: mustParsePublicHttpUrl("https://mcp.exa.ai/mcp"),
			authHeader: `Bearer ${apiKey}`,
		};
	}
	return { endpoint: mustParsePublicHttpUrl(DEFAULTS.searchEndpoint) };
}
