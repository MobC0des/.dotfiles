...
import { getWebToolsSettings, getExaEndpointConfig, SEARCH_DEPTHS, type ToolInputParseError } from "./settings.ts";
import {
...
function createSearchProvider(settings: WebToolsSettings["search"]): SearchProvider {
	switch (settings.provider) {
		case "exa": {
			const { endpoint, authHeader } = getExaEndpointConfig();
			return new ExaSearchProvider(endpoint, new FetchHttpTextClient(), authHeader);
		}
	}
}
